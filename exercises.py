...
test = input("Pleasae enter your password: ")
...
words_in = 'Hello World, We Are The People Who Loves Python'
res = sum(1 for char in words_in if char.isupper())
print(res)
...
for i in range(len(s1)):
	...
	for j in range(len(s1)):
		if s1[j:-1-i] in s2:
			print(s1[j:-i-1], i, s2)
			chunk = s1[j:-1-i]
			if chunk == chunk[::-1] and chunk != "":

				results.append(s1[j:-i-1])
print(results)
print("resluts = ", max(results))
